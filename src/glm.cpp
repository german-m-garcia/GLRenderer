...
NOTE: all the indexes of vertex, normal and texture are from 1 (not 0).
...
#if defined(_WIN32) || defined(_MSC_VER)
	s = strchr(dir,'\\'); //windows path separator is always '\\'
	if (s==NULL) // when there is no '\\', search '/'
	{
		s = strrchr(dir, '/');
	}
#else
...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	model->materials[0].name = strdup("glm_default");
...
glmFirstPass(GLMmodel* model, FILE* file)
{
	...
	group = glmAddGroup(model, "glm_default");
